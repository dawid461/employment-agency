...
const ContactForm = (props) => {
    ...
    return (props.trigger) ? (
        <>
            <div className='popup-form flex flex-col items-center justify-center' id='popUp'>
                <div
                    className="sm:max-w-xs md:max-w-lg xl:pb-10 xl:pt-5 bg-bgContactForm flex flex-col justify-center
                    items-center text-center">
                    <form onSubmit={handleSubmit(SendForm, onSubmitWithReCAPTCHA)} className="w-full max-w-lg">
                        <ReCAPTCHA
                            ref={recaptchaRef}
                            size="invisible"
                            sitekey="#"
                        ...
                        <div className="h-10 flex w-full justify-end mb-2">
                            <button className="closePopup mr-5" onClick={() => props.setTrigger(false)}>&times;</button>
                            {props.children}
                        </div>

                        <div className="flex w-full justify-center">
                            <h2 className="text-2xl font-bold text-grayTitle sm:mb-5 md:mb-7">Formularz kontaktowy</h2>
                        </div>

                        <div className="flex flex-wrap -mx-3">
                            <div className="w-full flex flex-col items-center px-3 mb-3">
                                <label htmlFor="name" className="block uppercase text-grayTitle text-xs font-bold mb-2">
                                    Imię i Nazwisko
                                </label>
                                <input name='fullName' {...register("fullName", {
                                    ...
                                    maxLength: {
                                       ...
                                       id="name" type="text" onChange={e => setName(e.target.value)}
                                />
                        ...
                        <div className="flex flex-wrap -mx-3 mb-2">
                            <div className="sm:h-12 md:h-full w-full flex flex-col items-center px-3">
                                <p className="mt-2 text-small">Wysyłając wiadomość zgadzam się na przetwarzanie moich
                                    danych osobowych przez Gregbud Wykonawstwo,z siedzibą na ul. Kazimierza Bartla 3,
                                    33-100 Tarnów w celu odpowiedzi na moją wiadomość, drogą telefoniczną lub poprzez
                                    e‑mail.*</p>
                            </div>
                        </div>
                        ...
                        <div className=" w-full flex flex-col items-center justify-center mt-2 mb-5">
                            ...
                            {showResults ?
                                <div>
                                    <p className=' mt-2 text-ld text-green font-semibold'>Wiadomość została wysłana.</p>
                                </div>
                                : null
                            }
                        </div>
